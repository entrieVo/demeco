...
export function impulseNoise<T extends Float32Array[] | Uint8ClampedArray>(
	signal: T,
	probability: number,
): T {
	if (
		...
		return audioImpulseNoise(signal, probability, probability) as unknown as T;
	else if (signal instanceof Uint8ClampedArray)
		return imageImpulseNoise(signal, probability) as unknown as T;
