...
export function colorNoise<T extends Float32Array[] | Uint8ClampedArray>(
	signal: T,
	...
	if (
		...
		return audioColorNoise(signal, relativeNoiseLevel) as unknown as T;
	else if (signal instanceof Uint8ClampedArray && width && height)
		return imageColorNoise(
			signal,
			relativeNoiseLevel,
			blurStrength,
...
function audioColorNoise(
	signal: Float32Array[],
	relativeNoiseLevel: number,
): Float32Array[] {
	let newData = signal.map((ch) => cloneFloat32Array(ch));

	newData = gaussianNoise(newData, relativeNoiseLevel);
...
function imageColorNoise(
	...
	newData.set(gaussianNoise(newData, relativeNoiseLevel));
	newData.set(
		gaussianBlur(newData, createKernel(kernelSize, sigmaBlur), height, width),
	);
